...
saveSignatureBtn.addEventListener('click', () => {
  ...
  .then(snapshot => {
    const orderData = order; // البيانات الأصلية للطلب
    
    const today = new Date().toISOString().split('T')[0];
    const accRef = ref(db, `accountingOrders/${today}`);
    return push(accRef, orderData);
  })
  .catch(err => {
    console.error('خطأ في حفظ الطلب في المحاسبة:', err);
  });
